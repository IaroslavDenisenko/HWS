...
let rendered = renderer.image { ctx in
    ...
    let leftYeye = CGRect(x: 250, y: 300, width: 150, height: 150)
    UIColor.black.setFill()
    ctx.cgContext.addEllipse(in: leftYeye)
    ctx.cgContext.drawPath(using: .fillStroke)
    
    let rightYeye = CGRect(x: 600, y: 300, width: 150, height: 150)
    UIColor.black.setFill()
    ctx.cgContext.addEllipse(in: rightYeye)
    ctx.cgContext.drawPath(using: .fillStroke)
}
